...
buildItem = function(e)
  ...
  for _, spellId in ipairs(weenie.SpellIds) do
    ...
    if bit.band(statFlags, EnchantmentFlags.Additive.ToNumber())>0 then
      desc = desc .. string.format(" increased by %.0f%s",spell.StatModVal*( float and 100 or 1), (int and "" or "%%"))
    elseif bit.band(statFlags, EnchantmentFlags.Additive_Degrade.ToNumber())>0 then
      desc = desc .. string.format(" reduced by %.0f%s",spell.StatModVal*( float and 100 or 1), (int and "" or "%%"))
    elseif bit.band(statFlags, EnchantmentFlags.Multiplicative.ToNumber())>0 then
      desc = desc .. string.format(" multiplied by %.0f%s",spell.StatModVal*( float and 100 or 1), (int and "" or "%%"))
    elseif bit.band(statFlags, EnchantmentFlags.Multiplicative_Degrade.ToNumber())>0 then
      desc = desc .. string.format(" multiplied by %.0f%s",(1-spell.StatModVal)*( float and 100 or 1), (int and "" or "%%"))
    end
...
end)local _imgui      = require("imgui")
local ImGui       = _imgui.ImGui
local views       = require("utilitybelt.views")
local io          = require("filesystem").GetScript()
local bit         = require("bit")
